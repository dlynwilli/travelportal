jQuery(document).ready(function() {	
	...
	jQuery( document ).on( "click", "#completeSave", function(){
		...
		jQuery.post(ajaxurl, data, function( response ) {
			if(!response)return;
			var post_type = response; // whether its the original post or revised post which is getting completed.
			...
			jQuery.post(ajaxurl, step_status_data, function( response ) {
				if(response){
					 ...
					 */
					if (response == "publish" && post_type == "complete_revision") { // if its a revision post, then change the status to draft
						new_post_status = "draft";
					}
					else if (post_type == "complete_original") { //if its the original post, simply change the status to the one from the workflow
						new_post_status = response;
					}

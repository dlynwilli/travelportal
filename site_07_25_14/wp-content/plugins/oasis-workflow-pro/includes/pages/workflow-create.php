	...
	<form id="wf-form" method="post" action="<?php echo network_admin_url('admin.php?page=oasiswf-admin');?>" >
					...
					<div class="postbox" >
						...
						<div class="move-div" id="workflow-define-div">
							...
							<table>
								...
								<tr>
									<td style="vertical-align: top;">
										<label><?php echo  __("Workflow applicable to :", "oasisworkflow");?></label>
									</td>
								</tr>
								<tr>
									<td>
									<?php
									 $str_n=""; $str_r="";
   				                 	 if( $wf_for_new_posts == 1 )$str_n = "checked=true" ;
								  	if( $wf_for_revised_posts == 1 )$str_r = "checked=true" ;
								  	?>
										<label><input type="checkbox" name="new_post_workflow"
   											value="1" <?php echo $str_n;?> />&nbsp;&nbsp;<?php echo __("new posts/pages", "oasisworkflow") ;?>
   									</label><br/>
									<label><input type="checkbox" name="revised_post_workflow"
   											value="1" <?php echo $str_r;?> />&nbsp;&nbsp;<?php echo __("revised posts/pages", "oasisworkflow") ;?>
   									</label>
									</td>
								</tr>
		...
		<div class="save-action-div">
			<?php if($wfeditable){?>
				...
				<input type="button" value="<?php echo __("Copy") ?>" class="button-primary save-to-copy" >
...
<?php
if (is_object($workflow)){
   echo "<div id='copy-workflow-create-check'>{$create_workflow->copy_workflow_create_check_html($workflow)}</div>" ;
}
?>

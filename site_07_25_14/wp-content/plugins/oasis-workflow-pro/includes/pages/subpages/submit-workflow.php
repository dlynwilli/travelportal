...
if( $workflow )
{
	...
	{
		$post_id = $_GET['post'];
		$check_revision = get_post_meta($post_id, 'oasis_original', true);
...
<div class="info-setting" id="new-workflow-submit-div">
	...
	<div>
			...
			<select id="workflow-select" style="width:200px;">
				...
				if( $workflow )
				{
					foreach ($workflow as $row)
					{
						...
						{
								$additional_info = unserialize($row->wf_additional_info);
		...
		<!-- Added publish date box for user ti choose future publish date. -->
		<?php if (!$is_revision):?>
      <div class="text-info left">
			<div class="left">
			...
			<div class="left">
				<input name="publish-date" id="publish-date" class="date_input" type="text"><input type="text" name="publish-hour" id="publish-hour" class="date_input wf-time" placeholder="hour" maxlength="2"><input type="text" name="publish-min" id="publish-min" class="date_input wf-time" placeholder="min"  maxlength="2">
				<button class="date-clear" ><?php echo __("clear", "oasisworkflow") ;?></button>
			</div>
			<br class="clear">
		</div>

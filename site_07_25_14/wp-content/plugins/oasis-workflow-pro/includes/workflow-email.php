...
class FCWorkflowEmail extends FCWorkflowBase
{
	...
	static function post_published_notification( $new_status, $old_status, $post )
	{
    	...
    	if ( $old_status != 'publish' && $new_status == 'publish' && $post->post_status != 'auto-draft'  && $post->post_status != 'inherit')
    	{
				$subject =  __("Your article has been published.", "oasisworkflow");
				$user = get_userdata($post->post_author);
				$to = $user->user_email;

				$msg = sprintf( __( '<div>Hello, <strong>%1$s</strong></div><p>Your article <a href="%2$s" titile="%3$s">%3$s</a> has been published on <a href="%4$s" titile="%5$s">%5$s</a></p><p>Thanks</p>', 'oasisworkflow' ), $user->display_name, esc_url( $post->guid ), $post->post_title, esc_url(get_bloginfo('url')),  get_bloginfo('name'));

...
class FCWorkflowBase
{
   ...
   static function get_action_history_by_post( $post_id )
   {
	...
	static function get_assigned_post($postid = null, $selected_user = null, $frm = "rows")
	{
		...
		if( $postid )
			$w = "WHERE (assign_actor_id = $user_id OR actor_id = $user_id) AND post_id = " . $postid . " order by A.due_date";
		else if(isset($user_id))
			$w = "WHERE assign_actor_id = $user_id OR actor_id = $user_id order by A.due_date" ;
		else
		   $w = " order by A.due_date";
...
/*     Workflow Reports              */
/*************************************/
include( OASISWF_PATH . "includes/workflow-submission-report.php" ) ;

/*************************************/
/*     Workflow Settings              */
/*************************************/
include( OASISWF_PATH . "includes/workflow-license.php" ) ;

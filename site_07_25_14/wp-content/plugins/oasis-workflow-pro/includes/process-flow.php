...
class FCProcessFlow extends FCWorkflowBase
{
	static function workflow_submit_check($selected_user)
	{
	   ...
	   //-------submit or make revision----------
		...
		if ( !empty( $post_var )) {
		   $post_status = get_post_status( $post_var );
			if ($post_status == "publish" && count( $rows ) == 0 ) {
			   return "makerevision";
			}
		}
	...
	static function save_action($data, $actors, $actionid=null, $actionfrm=null)
	{
      ...
      $wf_info = FCProcessFlow::get_step_by_id( $data["step_id"] ) ;
		...
		else if (!is_numeric( $actors ) && ($step_info->process == "assignment" || $step_info->process == "publish" )) //multiple actors are assigned in assignment step
		{
         $arr = explode("@", $actors) ;
   ...
   static function submit_post_to_workflow()
   {
	...
	static function change_workflow_status_to_complete()
	{
		...
		if( $iid ){
		   ...
		   // delete all the unsend emails for this workflow
			...
			$original_post_id = get_post_meta($_POST["post_id"], 'oasis_original', true);
			if (! empty( $original_post_id ))
			{
            // change the post status to used revision
            // we will update the contents of the original post via the currentrev to usedrev hook
            FCWorkflowPostDuplicate::change_status_to_owf_status( $_POST["post_id"], "currentrev" );
            ...
            echo "complete_revision";
			}
			else { // if this is the original post - act on it
   			if($_POST["immediately"] && $now_dt < $im_dt){
   				...
   				FCProcessFlow::copy_step_status_to_post($_POST["post_id"], $action->step_id, "complete", current_time('mysql')) ;
   			}
   			update_post_meta($_POST["post_id"], "oasis_is_in_workflow", 0); // set the post meta to 0, specifying that the post is out of a workflow.
   			echo "complete_original";
			}
		}
		exit();
	}
	...
	static function copy_step_status_to_post($postid, $stepid, $result, $immediately=null)
	{
		...
		if( $step ){
			...
			if( $step_status ){
				...
				if($immediately){
			      $im_dt = new DateTime($immediately);
			      $now_dt = new DateTime(current_time('mysql'));

					if($step_status == "publish" && $im_dt > $now_dt) {
					   $step_status = "future" ;
					}
	...
	static function get_original_post ( $post_id ) {
	...
	static function is_original_post () {
	   $oasiswf = FCProcessFlow::get_action_history_by_id( $_POST['oasiswf_id'] );
	   $original_post_id = FCProcessFlow::get_original_post ($oasiswf->post_id);
	...
	static function multi_abort_from_workflow()
	{
		global $wpdb;
		...
		foreach($post_ids as $post_id) :
			...
			$action = FCProcessFlow::get_action_history_by_id( $row->ID ) ;
		$comment[] = array(
						"send_id" => get_current_user_id(),
						"comment" => "Post/Page was aborted from the workflow."
					) ;
		...
		{
		   // delete all the unsend emails for this workflow
		   FCWorkflowEmail::delete_step_email($row->ID);
			$wpdb->update($action_history_table, array( "action_status" => "aborted",  "create_datetime" => current_time('mysql')), array( "ID" => $row->ID ) ) ;
			update_post_meta($action->post_id, "oasis_is_in_workflow", 0); // set the post meta to 0, specifying that the post is out of a workflow.
		}
		endforeach;
		echo true;
